...
def conv2d_vgg(input_,data_wb,name="conv2d"):
  with tf.variable_scope(name):
    #
    weights, bias = data_wb
    weights = np.transpose(weights, (1, 0, 2, 3))
    bias = bias.reshape(-1)
    init_w = tf.constant_initializer(weights)
    init_b = tf.constant_initializer(bias)
    w = tf.get_variable('w', weights.shape,initializer=init_w)
    conv = tf.nn.conv2d(input_, w, strides=[1, 1, 1, 1], padding='SAME')

    biases = tf.get_variable('biases', bias.shape, initializer=init_b)
